  ...
  <div>
    <div>
      ...
      <Legend
        ...
        class="viz-legend pb-3 mb-5, mt-3"
      />

      <!-- The total change -->
      <div class="total-change text-center mt-3">
        Total Spending Cut:
...
export default {
  ...
  components: { Legend, VueGoodTable, RadioButtonToolbar },
  data() {
    return {
  ...
  computed: {
    ...
    tableColumns() {
      ...
      cols.push(
        {
          label: this.selectedComparison,
          field: this.selectedComparison,
          type: "number",
          formatFn: formatFn
        },
        {
    ...
    tableRows() {
      ...
      for (let [k, v] of grouped) {
        ...
        // Total baseline
        row[this.selectedComparison] = d3.sum(
          v,
          d => d[this.selectedComparison]
        );

        // Total 2021
        row[`2021 (Adopted)`] = d3.sum(v, d => d[`2021 (Adopted)`]);

        // Diff
        row["diff"] = row[`2021 (Adopted)`] - row[this.selectedComparison];

        // Don't add rows for empty line
        if (this.isEmptyLineItem(row)) continue;

        // Percent diff
        row["percent_diff"] = row["diff"] / row[this.selectedComparison];
        ...
        for (let i = 0; i < v.length; i++) {
          ...
          // 2021
          child[`2021 (Adopted)`] = v[i][`2021 (Adopted)`];

          // baseline
          child[this.selectedComparison] = v[i][this.selectedComparison];

          // Differnce
          child["diff"] =
            v[i][`2021 (Adopted)`] - v[i][this.selectedComparison];

          // Percent diff
          child["percent_diff"] = child["diff"] / v[i][this.selectedComparison];
    ...
    tooltipConfig() {
      return [
        ...
        {
          title: this.selectedComparison,
          data_field: this.selectedComparison,
          format_string: ",.3s"
        },
        {
          title: `2021 (Adopted)`,
          data_field: `2021 (Adopted)`,
          format_string: ",.3s"
        },
        {
          title: "Net Change",
  ...
  watch: {
    ...
    selectedComparison(newValue, oldValue) {
  ...
  methods: {
    ...
    isEmptyLineItem(d) {
      return (d[`2021 (Adopted)`] == 0) & (d[this.selectedComparison] == 0);
    },
    hasSummaryRow() {
    ...
    addSummaryRow() {
      ...
      let baseline = d3.sum(this.tableRows, d => d[this.selectedComparison]);
      let adopted = d3.sum(this.tableRows, d => d[`2021 (Adopted)`]);
      let diff = adopted - baseline;
      let percent_diff = diff / baseline;
      ...
      row =
        row +
        `<th class="vgt-row-header vgt-right-align">${formatFn(baseline)}</th>
        <th class="vgt-row-header vgt-right-align">${formatFn(adopted)}</th>
    ...
    getRadiusScale(data, maxPixels) {
      ...
      */
      ...
      let diffMax = d3.max(data, d =>
        Math.abs(d[`2021 (Adopted)`] - d[this.selectedComparison])
      );
      return d3
        .scalePow()
        .exponent(0.5)
    ...
    getSpendingDiff(d, absFlag) {
      ...
      */
      ...
      let diff = d[`2021 (Adopted)`] - d[this.selectedComparison];
    ...
    getSpendingDiffPercent(d, absFlag) {
      ...
      */
      ...
      let toret = diff / d[this.selectedComparison];
      if (diff == 0 && d[`2021 (Adopted)`] == 0) toret = 0;
      return toret;
    },
      ...
      if (singleBubble) {
        ...
        labels = Array.from(
          rollup(
            this.rawData,
            v =>
              d3.sum(v, d => d[`2021 (Adopted)`] - d[this.selectedComparison]),
            d => d[groupby]
          )
        )
    ...
    ticked() {
      ...
      */
      ...
      if (
        ...
        // add lines
        // let t2 = d3.transition().duration(1000);
          ...
          if ("format_string" in cur_tooltip) {
            if (cur_tooltip.data_field == "percent_diff") {
              ...
              value_formatted = netChangeFormatFn(
                d[`2021 (Adopted)`] - d[this.selectedComparison]
              );
            } else {
              value_formatted = `$${d3
...
<style>
.budget-explorer-toolbar {
  border-top: 2px solid #deedfc;
  margin-top: 5rem;
}
